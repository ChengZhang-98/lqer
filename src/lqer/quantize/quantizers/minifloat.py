...
def _minifloat_denorm_quantize(
    ...
    exponent = clamp_ste(exponent, exponent_min, exponent_max)
    ...
    shifted_mantissa = round_ste(mantissa * shift)
    # clip the integer mantissa.
    shifted_mantissa = clamp_ste(
...
def _minifloat_ieee_quantize(
    ...
    shifted_mantissa = is_normal*clamp_ste(round_ste(mantissa*shift-shift), shifted_mantissa_min, shifted_mantissa_max) +\
        (~is_normal)*clamp_ste(round_ste(mantissa*shift/2), shifted_mantissa_min, shifted_mantissa_max)
