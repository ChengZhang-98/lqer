...
from .utils import block, clamp_ste, round_ste, unblock
...
def _block_fp_quantize(
    ...
    per_block_exponent = clamp_ste(per_block_exponent, exponent_min, exponent_max)
    # mantissa
    per_block_mantissa = per_block_value / 2**per_block_exponent
    shift = 2**mantissa_bits
    per_block_mantissa_integer = clamp_ste(
        round_ste(per_block_mantissa * shift), 0, mantissa_integer_max
    )
